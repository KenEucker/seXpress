...
function InitNotifications(initial, notificationsOpts = {}) {
    ...
    if (this.config.notifications.enabled) {
		...
		this.hook(notificationHooks.notify, (notification) => {
			if (notification.subscription) {
				sendNotificationToSubscription(notification.subscription, notification.title, notification.message)
			} else if (notification.domain) {
				sendNotificationToAllSubscribers(notification.title, notification.message, notification.domain)
			}
		})
        ...
        this.notifications = {
            webpush,
            sendNotificationToAllSubscribers: (title, message, domain) => {
                this.hook(
                        ...
                        if (subscribers?.length) {
                            subscribers.forEach((subscriber) => {
								if (domain && subscriber.domain === domain) {
                                	sendNotificationToSubscription(subscriber.subscription, title, message)
								}
                            })
                        }
                    },
