...
function InitSSL(initial, sslOpts = {}) {
    ...
    if (this.config.ssl.enabled) {
        ...
        if (!this.config.ssl.sites || this.config.ssl.sites.length === 0) {
            const generateSslHostnames = (host) => {
                const altnames = [host]
                Object.keys(this.config.subdomains).forEach((subdomain) => {
                    if (subdomain !== 'index') {
                        altnames.push(`${subdomain}.${host}`)
                    }
                ...
                Object.keys(this.coreSubdomains).forEach((index) => {
                    ...
                    if (altnames.indexOf(subdomainHost) === -1) {
                        altnames.push(subdomainHost)
                    }
                })

                return { subject: host, altnames }
            }

            this.config.ssl.sites = []
            const hosts = Array.isArray(this.config.host) ? this.config.host : [this.config.host]

            hosts.map((host) => {
				if (this.config.ssl.wildcardedHosts) {
                	this.config.ssl.sites.push(`*.${host}`)
                	this.config.ssl.sites.push(host)
				} else {
                	this.config.ssl.sites.push(generateSslHostnames(host))
				}
            })
        }
        ...
        switch (this.config.ssl.strategy) {
            ...
            case 'greenlock':
                const manuallyGenerateGreenlockConfig = (configDir) => {
                    if (!fs.existsSync(configDir)) {
                        ...
                        fs.writeFileSync(configFile, JSON.stringify({ sites: this.config.ssl.sites }))
                    }
                }

                const useGreenlockApiToAddSites = (serverOpts) => {
                    const greenlockApi = require('@root/greenlock')
                    const gl = greenlockApi.create(serverOpts)

                    gl.manager.defaults({
                        agreeToTerms: true,
                        subscriberEmail: serverOpts.maintainerEmail,
                    })

                    this.config.ssl.sites.map(gl.add)
                }
                ...
                this.__ssl = () => {
					...
					if (this.config.ssl.wildcardedHosts) {
						serverOpts.challenges = {
							'dns-01': require('acme-dns-01-cli').create({ debug: true }),
						}
					}

                    if (this.config.ssl.manuallyGenerateGreenlockConfig) {
                        manuallyGenerateGreenlockConfig(configDir)
                    } else {
                        useGreenlockApiToAddSites(serverOpts)
                    }

                    this.config.ssl.opts = serverOpts
                    const greenlockServer = greenlock.init(serverOpts)
					
                    greenlockServer.serve(this.app)
