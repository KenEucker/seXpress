            ...
            switch (this.config.ssl.strategy) {
				...
				case 'greenlock':
					this.__ssl = () => {
						...
						const serverOpts = {
							packageRoot: config.folders.appFolder,
							configDir: join(config.folders.configFolder, "greenlock.d"),
	
							// Staging for testing environments
							staging: config.debug,
					
							// contact for security and critical bug notices
							maintainerEmail: config.author.email,
					
							// whether or not to run at cloudscale
							cluster: false,
							
							// for an RFC 8555 / RFC 7231 ACME client user agent
							packageAgent: `${config.name}/${config.version}`,
						}
