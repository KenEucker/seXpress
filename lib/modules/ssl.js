    ...
    if (this.config.ssl.enabled) {
		...
		if (!this.config.ssl.sites) {
			...
			Object.keys(this.config.subdomains).forEach((subdomain) => {
				altnames.push({ subject: `${subdomain}.${this.config.host}`})
			})
			this.config.ssl.sites = [
				{ subject: this.config.host, altnames}
			]
		}
            ...
            switch (this.config.ssl.strategy) {
				...
				case 'greenlock':
					...
					if (!fs.existsSync(configDir)) {
						...
						this.log.status(`creating greenlock.d config folder and autopopulating ssl config`, {sites: this.config.ssl, configFile})
