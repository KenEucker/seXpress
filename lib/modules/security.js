...
function InitSecurity(initial, securityOpts = {}) {
    ...
    if (this.config.ssl.enabled) {
        const hostnameSources = Array.isArray(this.config.host)
            ? this.config.host.map((h) => [h])
            : [this.config.host]
        const contentSecurityPolicy = merge(
            {
                directives: {
                    ...
                    connectSrc: [`'self'`, `wss://localhost:9856`, ...[hostnameSources]],
        ...
        this.app.use(
            helmet({
                ...
                contentSecurityPolicy: this.config.debug || this.config.csp.disabled ? false : contentSecurityPolicy,
            }),
        )
    }
}
