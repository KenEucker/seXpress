        ...
        const corsOpts = util.merge(
            {
                ...
                origin: (origin, callback) => {
					...
					/// If the origin subdomain is the api
					/// TODO: check the credentials of the caller here?
					const originSubdomain = util.getSubdomainPrefix(this.config, origin)
					if (originSubdomain === 'api') return callback(null, true)
    ...
    if (this.isSecure()) {
        this.app.use(
            helmet({
				...
				/// TODO: fix nonces in production and then uncomment these lines.
				// {
                //     directives: {
                //         defaultSrc: [`'self'`],
