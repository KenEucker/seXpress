                        ...
                        .on('entry', (entry) => {
                            ...
                            const copyFileFromArchive = (destination, filePath = '') => {
                                if (entry.type === 'Directory') {
                                    mkdirp.sync(join(destination, filePath))
                                } else {
                                    entry.pipe(createWriteStream(join(destination, filePath)))
                                }
                            }
                            ...
                            if (
                                ...
                                entry.pipe(
                                    createWriteStream(join(targetAppDir, 'config', 'config.js')),
                                )
                            } else if (
                                ...
                                /// copy the controller files over to the /controllers folder in the root applicaiton
                                copyFileFromArchive(targetAppDir, outputPath)
                            } else if (
