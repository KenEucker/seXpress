    ...
    if (this.config.generate.enabled && this.config.generate.source) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!existsSync(configFilePath) || this.config.generate.sync) {
                        ...
                        .on('close', () => {
							...
							if (this.config.generate.importDependencies) {
								...
								if (existsSync(sourcePackageJsonPath) && existsSync(thisPackageJsonPath)) {
									...
									writeFileSync(thisPackageJsonPath, JSON.stringify(packageJson, null, 2))
								}
                ...
                if (existsSync(tempTemplateArchive)) {
                    /// WARNING: this will overwrite the local copy of these files. Is the user sure they want to lose any changes to those files?
                    this.log.status(
                    ...
                    await createReadStream(tempTemplateArchive)
                        ...
                        .on('entry', (entry) => {
                            ...
                            if (
                                ...
                                /// copy the lib files over to the /templates folder in the root applicaiton
                                copyFileFromArchive(targetAppDir, outputPath)
                            } else if (
                                this.config.generate.copyPublic &&
                                outputPath.indexOf('public/') === 0
                            ) {
                                /// copy the lib files over to the /public folder in the root applicaiton
                                copyFileFromArchive(targetAppDir, outputPath)
                            } else {
                                entry.autodrain()
                            }
