                    ...
                    return createReadStream(tempSourceArchive)
                        ...
                        .on('entry', (entry) => {
                            if (
                                ...
                                /// If this is the config.js file, copy it out to the root, if not copy it into the temp/config folder
                                const outputPath = entry.path.endswith('.js')
                                    ? 'config.js'
                                ...
                                console.log('would copy lib file', entry.path)
                                /// TODO: copy over all lib files to the root lib folder
                                entry.autodrain()
                            } else if (
                                this.config.generate.copyControllers &&
                                entry.path.indexOf('/controllers/') !== -1
                            ) {
                                /// TODO: copy over all controllers files to the controllers folder
                                console.log('would copy controllers file', entry.path)
                                entry.autodrain()
                            } else if (
                                this.config.generate.copyControllers &&
                                entry.path.endswith('/app.js')
                            ) {
                                console.log('would copy app file', entry.path)
