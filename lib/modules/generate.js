				...
				try {
					...
					if (this.config.generate.templatesSource) {
						...
						this.log.status(
							...
							writeFileSync(
							tempTemplateArchive,
							await download(freshTemplatesSourceUrl),
						)
					}
					...
					if (tempSourceArchiveExists) {
						...
						for await (const entry of sourceArchiveFiles) {
							if (!extractConfigFiles(entry, targetAppDir, this.config.generate.copySourceConfig)) {
								entry.autodrain()
							} else {
					...
					if (tempTemplateArchiveExists) {
						...
						const tempArchiveFiles = createReadStream(tempTemplateArchive)
							.pipe(unzipper.Parse({forceStream: true}))
							.on('close', () => {
					...
					if (this.generate.copyPrivateConfig && existsSync(privateConfigFile)) {
						copyFileSync(privateConfigFile, join(targetAppDir, 'config', 'private.config.json'))
					}

					/// Restart/Reinitialize our app
					return resolve('configuration loaded, restart required')
				} catch (err) {
