        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (!existsSync(configFilePath) || this.config.generate.sync) {
				...
				if (this.config.generate.templatesSource) {
                    writeFileSync(tempTemplateArchive, await download(this.config.generate.templatesSource))
				}
				...
				const extractConfigFiles = (entry, targetAppDir) => {
						...
						if (
							...
							copyFileFromArchive(targetDir, outputPath)
						} else if (
                    ...
                    await createReadStream(tempSourceArchive)
                        ...
                        .on('close', () => {
							...
							if (this.config.generate.importDependencies) {
								...
								if (existsSync(sourcePackageJsonPath) && existsSync(thisPackageJsonPath)) {
									const packageJson = require(thisPackageJsonPath)
									const sourcePackageJson = require(sourcePackageJsonPath)
									packageJson.dependencies = packageJson.dependencies || {}

									Object.keys(sourcePackageJson.dependencies).forEach((dependencyName) => {
										...
										} else if (!this.config.generate.keepOlderDependencies) {
											/// defaulting to the opposite of a flag that begins in an undefined state
											packageJson.dependencies[dependencyName] = dependencyVersion
										}
									})

									writeFileSync(thisPackageJsonPath, JSON.stringify(packageJson, null, `\t`))
								}

							}
							
							if (!tempTemplateArchiveExists || !this.config.generate.copyTemplateConfig) {
