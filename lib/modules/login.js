    ...
    if (this.config.authentication.enabled || this.config.login.enabled) {
        const getRequestUserIDFromProvidedUser = (user) => {
            ...
            switch (user.provider) {
                ...
                default:
                    this.log.error(
                        `cannot serialize user for login provider [${user.provider}]`,
                        { user, profile, userUniqueID, }
                    )
                    break
            }
        ...
        if (this.config.login.enabled) {
            ...
            const userAuthed = (req, res) => {
                if (!!req.isAuthenticated()) {
                    self.log.status(`👋 user logged in`, { user: req.user, session: req.session, passport: req.session.passport })
            ...
            const getLoginViewController = (view = 'index', restrict = false) => {
                return this.requestHandler(
                    function loginHandler(subdomain, req, res, host, next) {
						...
						console.log('ATTEMPTING LOGIN', {req, auth: req.isAuthenticated()})
                        if (view === 'index' && req.isAuthenticated()) {
                            ...
                            console.log('redirecting to profile getLoginViewController')
                            return res.redirect(profileUrl)
                        }

                        if (view === 'profile' && !req.isAuthenticated()) {
                            console.log('cannot show profile redirecting getLoginViewController')
