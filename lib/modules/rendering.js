...
function InitRendering(initial, renderingOpts = {}) {
	...
	const compileStyleAndInject = (target, templateSassFilePath, injectionTarget = '</head>', before = true) => {
		if (fs.existsSync(templateSassFilePath)) {
			const styleRendered = sass.renderSync({ file: templateSassFilePath })
			...
			target = target.replace(injectMarker, before ? `<style>\n${css}\n</style>${injectionTarget}` : `${injectionTarget}<style>\n${css}\n</style>`)
		}
