...
function InitRendering(initial, renderingOpts = {}) {
    ...
    const compileStyleAndInjectThenRender = (targetFilePath, templateSassFilePath, opts = {}) => {
        ...
        if ((!targetString || this.config.debug) && fs.existsSync(targetFilePath)) {
            ...
            if (this.config.debug && targetFileIsIndex) {
                this.log.debug(moduleName, `Injecting dev reload script into rendered output`)
                renderedString = injectIntoString({
                    haystack: renderedString,
                    needle: '</body>',
            ...
            this.log.debug(moduleName, `using cached version of this view/template`, cacheKey)
        }
    ...
    if (this.config.rendering.enabled) {
            ...
            switch (rendererToUse) {
                case 'liquid':
                    ...
                    const liquidOpts = merge(
                        {
                            ...
                            customFilters: {
                                ...
                                stylesheet_tag: (href, nonce) =>
                                    `<link href="${href}" rel="stylesheet" type="text/css" media="all" nonce="${nonce}"/>`,
                                javascript_tag: (src, nonce) =>
                                    `<script src="${src}" rel="stylesheet" type="text/javascript" nonce="${nonce}"></script>`,
                                img_tag: (src, alt) =>
                    ...
                    Object.keys(liquidOpts.customFilters).forEach((filter) => {
                        ...
                        this.log.debug(moduleName, `🧬 adding custom liquid filter [${filter}]`)
        ...
        this.log.debug(moduleName, '🎨 supporting rendering of views in the folder', {
