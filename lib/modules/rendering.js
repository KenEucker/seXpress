...
module.exports = function InitRendering(initial, renderingOpts = {}) {
	const util = require('../util')(this.config.appRoot)
    this.config.rendering = this.getCoreOpts(moduleName, util.merge(renderingOpts, {
                    ...
                    const liquidOpts = util.merge(
                        {
                            ...
                            customFilters: {
								...
								`${container  ? `<${container}${classes ? ` class="${classes}"`: ''}>` : ''}<a href="hooks.host/${hook}" ${!container && classes ? ` class="${classes}"`: ''}${target ? `target="${target}"` : ''}>${text}</a>${container  ? `</${container}>` : ''}`,
                            },
                        },
