    ...
    const compileStyleAndInjectThenRender = (targetFilePath, templateSassFilePath, opts = {}) => {
        ...
        if (!targetString && fs.existsSync(targetFilePath)) {
            ...
            if (fs.existsSync(templateSassFilePath)) {
                ...
                renderedString =
                    renderedString.indexOf(opts.injectMarker) !== -1
                        ? renderedString
                        : `${opts.injectMarker}${renderedString}`
                    ...
                    const liquidOpts = merge(
                        {
                            ...
                            customFilters: {
                                console_log: (data, nonce) => `<scrip nonce="${nonce}">console.log(${data})</scrip>`,
                                stylesheet_tag: (href, nonce) =>
                                    `<link nonce="${nonce}" href="${href}" rel="stylesheet" type="text/css" media="all" />`,
                                javascript_tag: (src, nonce) =>
                                    `<scrip nonce="${nonce}" src="${src}" rel="stylesheet" type="text/javascript"></scrip>`,
                                img_tag: (src, alt, nonce) =>
                                    `<img nonce="${nonce}" src="${src}" alt="${alt ? alt : src}">`,
                                meta_name: (name, content) =>
                                    `<meta name="${name}" content="${content}">`,
                                meta_prop: (name, content) =>
                                    `<meta property="${name}" content="${content}">`,
								asset_url: (asset) => `/public/${asset}`,
								safe_script: (inner, nonce) => `<scrip nonce="${nonce}">${inner}</scrip>`,
								safe_style: (inner, nonce) => `<style nonce="${nonce}">${inner}</style>`,
                                sexy_api_script: (nonce) =>
                                    `<scrip nonce="${nonce}" src="/public/js/${this.config.api.apiFilename}" rel="stylesheet" type="text/javascript"></scrip>`,
