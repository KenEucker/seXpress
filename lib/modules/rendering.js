...
function InitRendering(initial, renderingOpts = {}) {
	...
	/// Cached response
    const compileStyleAndInjectThenRender = (
		targetFilePath,
		...
		templateSassFilePath = templateSassFilePath ? templateSassFilePath : targetFilePath.replace(
			`.${opts.rendererExt}`,
			`.${opts.stylerExt}`,
		)

		/// Set the cache key for this specific template
		const cacheKey = opts.cacheKey ? opts.cacheKey : `${moduleName}::<${targetFilePath.replace(this.config.folders.appFolder, '')}|${templateSassFilePath.replace(this.config.folders.appFolder, '')}>`
		let targetString = this.cache.get(cacheKey)
		console.log({opts, targetFilePath, templateSassFilePath})
		
		if (!targetString && fs.existsSync(targetFilePath)) {
			...
			if (fs.existsSync(templateSassFilePath)) {
				...
				this.log.debug(
					!cssString ? 'erorr rendering embedded style' : 'styles rendered and embedded into view',
					{
						templateSassFilePath,
					},
				)

				/// In order to add styles to the end of the rendered text, we need to mark where the end is
				const injectMarker = '%%STYLEINJECTEDHERE%%'

				renderedString = renderedString.replace(opts.injectionTarget, opts.injectMarker)
				renderedString = renderedString.indexOf(opts.injectMarker) !== -1 ? renderedString : `${opts.injectMarker}${renderedString}`
				renderedString = renderedString.replace(
					opts.injectMarker,
					opts.before
						? `<style>\n${cssString}\n</style>${opts.injectionTarget}`
						: `${opts.injectionTarget}<style>\n${cssString}\n</style>`,
				)
			}
    ...
    if (this.config.rendering.enabled) {
            ...
            switch (rendererToUse) {
                case 'liquid':
                    ...
                    const liquidOpts = merge(
                        {
                            ...
                            customFilters: {
                                ...
                                sexy_api_script: () =>
                                    `<script src="/public/js/${this.config.api.apiFilename}" rel="stylesheet" type="text/javascript"></script>`,
                                sexy_data: (name) => `
								<script>
								((isSet) => {
									if (!isSet) {
					...
					sexyRenderFunction = function sexyLiquidRenderer(
						...
						// 	`.${stylerToUse}`,
						// )
                        const viewFileString = compileStyleAndInjectThenRender(viewFilePath, {rendererExt: 'liquid'})

						console.log({viewFileString})
						liquid.parseAndRender(viewFileString, viewData).then((rendered) => {
					...
					// 		renderer: liquid.parseAndRenderSync,
					// 		viewData
					// 	})
        ...
        overrideRenderer = !(typeof overrideRenderer === 'array' || typeof overrideRenderer === 'object') ? [overrideRenderer] : overrideRenderer
		/// load all of the remaining engines
		overrideRenderer.forEach((engine) => {
			setOverrideViewEngine(engine)
		})
