...
function InitAuthentication(initial, authenticationOps = {}) {
	...
	*/
    ...
    const configurePassportMiddleware = (
		...
		const authTokens = this.authTokens[subdomain][middlewareConfigName]
        ...
        const setTokensForSubdomain = (subdomain, accessToken, refreshToken, profile) => {
			const currentTokens = authTokens[subdomain][middlewareConfigName]
        ...
        const setTokens = (accessToken, refreshToken, profile, subdomain) => {
            ...
            for (const sub of subdomainsToSet) {
                if (authTokens[sub] && Object.keys(authTokens[sub]).length) {
        ...
        const subdomainGetTokenRequest = (req, res) => {
            if (this.isValidRequestOrigin(req)) {
                ...
                const subdomainAuth = authTokens[subdomain][middlewareName]
                ...
                this.log.debug(moduleName, `${middlewareName} getToken response`, {
                    subdomain,
                    subdomainAuth,
        ...
        for (const subdomain of Object.keys(this.config.subdomains)) {
            ...
            authTokens[subdomain][middlewareName] = {}
            ...
            authTokens[subdomain][middlewareName].opts = subdomainStrategyOpts
        }
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
            ...
            const uniqueClientStrategyOpts = subdomainIsDefault
                ? authTokens[firstSubdomainSet][middlewareName]
                : authTokens[firstSubdomainSet][middlewareName].opts
            ...
            this.log.debug(moduleName, `auth strategy [${middlewareName}] created for client ID:`, {
        ...
        const refreshTokens = () => {
            const refreshToken = (subdomain) => {
                const theRefreshTokenToUse = authTokens[subdomain][middlewareName][
                    keys.refreshToken
                ]
                this.log.status(
		...
		this.authTokens[subdomain][middlewareConfigName] = authTokens
    }
			...
			if (strategyConfigName.indexOf(imgurAuthPrefix) === 0) {
				configurePassportMiddleware(
					...
					(profile, subdomain) => {
						...
						return (
							profile !== null &&
							profile.username === authTokens.imgurEmailAddress
						)
					},
				)
			}
			...
			if (strategyConfigName.indexOf(redditAuthPrefix) === 0) {
				configurePassportMiddleware(
					...
					(profile, subdomain, authTokens) => {
						...
						return (
							profile !== null &&
							profile.username === authTokens.redditEmailAddress
						)
					},
				)
			}
			...
			if (strategyConfigName.indexOf(googleAuthPrefix) === 0) {
				configurePassportMiddleware(
					...
					(profile, subdomain, authTokens) => {
						...
						return (
							profile !== null &&
							profile.username === authTokens.googleEmailAddress
						)
					},
				)
			}
