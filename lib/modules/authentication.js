...
async function InitAuthentication(initial, authenticationOps = {}) {
	...
	*/
            ...
            try {
				...
				await server
					.open((err) => {
						...
						this.app.get(this.config.session.redis.route, isUserAuthed, session(cookieOpts), sendCookieInfo) 
						this.app.post(this.config.session.redis.route, isUserAuthed, session(cookieOpts), sendCookieInfo)

						this.log.info(`⛺️ redis storage enabled for authentication on route [${this.config.session.redis.route}]`)
					})
            } catch (e) {
            ...
            switch (scheme.name) {
				...
				case 'json':
					...
					const jsonValidator = (u, p, d) => {
						const usernamePassed = !!jsonUsername
							? RegExp(jsonUsername).test(u)
							: true
						const passwordPassed = !!jsonPassword
							? RegExp(jsonPassword).test(p)
							: true
    ...
    const configurePassportMiddleware = (
        ...
        middlewarePassport = middlewarePassport || getInitializedPassport()
			...
			this.app.post(
				...
				(req, res, next) => {
					...
					return attemptToAuthorizeForSubdomains(req, accessToken, refreshToken, profile, (error, userID) => {
						...
						this.log.status(`authentication propogation for subdomain ${self.isAuthenticated(req) ? 'succeeded' : 'failed'}`, {
							subdomain,
							profile,
						})
