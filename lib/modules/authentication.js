...
module.exports = function (authenticationOps = {}) {
	...
	*/
    ...
    const configurePassportMiddleware = (
        ...
        const defaultStrategyOpts = convertAuthOptsToStrategyOpts(securityOpts[middlewareName], securityOpts)
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
            ...
            const validSubdomains = validSubdomainsIsString ? [uniqueClientStrategies[clientID]] : uniqueClientStrategies[clientID]
			...
			this.app.get(`/auth/${middlewareName}`,
    ...
    if (this.config.authentication.imgur && this.config.authentication.imgur.imgurClientID) {
        configurePassportMiddleware(this.config.authentication,
            'imgur',
            ImgurStrategy,
            (profile, subdomain) => {
