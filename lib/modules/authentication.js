        ...
        authenticationStrategies.forEach((strategyConfigName) => {
            if (strategyConfigName.indexOf(redditAuthPrefix) === 0) {
                configurePassportMiddleware(
                    ...
                    (req, res, next, opts = {}) => {
						const { subdomain } = res.locals
						const stateFromIndexRequest = this.authTokens[subdomain] && this.authTokens[subdomain][strategyConfigName] ? this.authTokens[subdomain][strategyConfigName].state : undefined
						console.log({subdomain, stateFromIndexRequest})
