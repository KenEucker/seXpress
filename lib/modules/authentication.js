...
function InitAuthentication(initial, authenticationOps = {}) {
    ...
    const { Strategy: ImgurStrategy, userProfile: setImgurUserProfile } = require('passport-imgur')
    const { Strategy: RedditStrategy, userProfile: setRedditUserProfile } = require('passport-reddit')
    const { Strategy: GitHubStrategy, userProfile: setGithubUserProfile } = require('passport-github')
    const { Strategy: InstagramStrategy, userProfile: setInstagramUserProfile } = require('passport-instagram')
    const { Strategy: GoogleStrategy, userProfile: setGoogleUserProfile } = require('passport-google-oauth2')
	...
	*/
    ...
    const configurePassportMiddleware = (
		...
		authStrategyMethod,
		authSetUserProfile = (i,d) => d('user profile setter not defined'),
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
			...
			this.app.post(
				...
				(req, res, next) => {
					...
					console.log(`calling attemptToAuthorizeForSubdomains`, {subdomain})
					return attemptToAuthorizeForSubdomains(req, accessToken, refreshToken, profile, (error, userID) => {
						...
						if (userID) {
							return authSetUserProfile(accessToken, (err, profile) => {
								console.log({err, profile})
								this.log.status(`authentication propogation for subdomain ${!err ? 'succeeded' : 'failed'}`, {
									subdomain,
									profile,
								})
							})
						...
						this.log.status(`authentication propogation for subdomain ${req.isAuthenticated() ? 'succeeded' : 'failed'}`, {
							subdomain,
							profile,
						})
