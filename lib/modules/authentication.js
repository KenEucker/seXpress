                ...
                this.app.get(
                    ...
                    this.requestHandler((req, res, next) => {
                        console.log('authCallback', {
                            call: authStrategyOpts.callbackURL,
                            q: req.query,
                        })
			...
			this.app.post(
				...
				(req, res, next) => {
					...
					return attemptToAuthorizeForSubdomains(req, accessToken, refreshToken, profile, (error, userID) => {
						console.log('attemptToAuthorizeForSubdomains RETURNED', {error, userID})
        ...
        const imgurUserValidator =
            typeof this.config.authentication.imgur.imgurUserValidator === 'function'
                ...
                : (profile, subdomain, authTokens) => {
					const imgurEmailAddress = this.config.subdomains[subdomain].imgur ? this.config.subdomains[subdomain].imgur.imgurEmailAddress : null

					if (!imgurEmailAddress) {
						this.log.error(`cannot validate imgur user because account email address is not set`, this.config.subdomains[subdomain])
						return false
					}
                      ...
                      if (imgurEmailAddress)
                          return (
                              ...
                              profile.username.toLocaleLowerCase() ===
                                  imgurEmailAddress.toLocaleLowerCase()
                          )
