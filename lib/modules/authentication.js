	...
	*/
    ...
    const configurePassportMiddleware = (
        ...
        const getRequestUserIDFromProvidedUser = (user) => {
            let profile = {}
            let userUniqueID = user.username ? user.username : userid
        ...
        const setTokensForSubdomain = (subdomain, accessToken, refreshToken, profile) => {
			...
			console.log({currentTokens, subdomain, accessToken, refreshToken})
        ...
        const attemptToAuthorizeForSubdomains = (req, accessToken, refreshToken, profile, done) => {
			...
			// )
			console.log({subdomain, accessToken, refreshToken, profile})
			if (subdomain === 'index') {
				/// TODO: send uest requests with teh same information to all subdomains as well
				/// TODO: recreate callback url with access and refresh tokens for subdomains
				const subdomainsToSet = Object.keys(this.config.subdomains).filter(s=>s !== 'index')
				console.log(`sending callback to all subdomains configured for auth`, {subdomainsToSet})
            	for (const sub of subdomainsToSet) {
					const queryString = Object.keys(req.query).reduce((o, k) => { o = `${o}&${k}=${req.query[k]}` ; return o}, '').substring(1)
					const subdomainCallbackUrl = `${this.authTokens[sub][middlewareConfigName].opts.callbackURL}?${queryString}`
					
					req.uest({
						method: 'GET',
						url: subdomainCallbackUrl,
						baseUrl: this.getBaseUrl(undefined, undefined, sub),
						...
						console.log("HELLLOOOOO", {sub, middlewareConfigName, subdomainCallbackUrl, err})
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
            ...
            if (typeof customCallback === 'function') {
                ...
                this.app.get(
                    ...
                    this.requestHandler((req, res, next) => {
						console.log('callback', {call: authStrategyOpts.callbackURL, q: req.query})
                        return next()
                    }, interceptSubdomains),
                    (req, res, next) => {
            ...
            if (typeof customAuthCallback === 'function') {
                this.app.get(
                    ...
                    this.requestHandler((req, res, next) => {
						console.log('authCallback', {call: authStrategyOpts.callbackURL, q: req.query})
        ...
        const imgurUserValidator =
            typeof this.config.authentication.imgur.imgurUserValidator === 'function'
                ...
                : (profile, subdomain, authTokens) => {
                      if (authTokens.opts.imgurEmailAddress)
                          return (
                              profile !== null &&
                              profile.username.toLocaleLowerCase() ===
                                  authTokens.opts.imgurEmailAddress.toLocaleLowerCase()
                          )
            ...
            if (strategyConfigName.indexOf(redditAuthPrefix) === 0) {
                configurePassportMiddleware(
                    ...
                    (req, res, next, opts = {}) => {
						...
						this.authTokens[subdomain][strategyConfigName].state = req.session.state = sessionState

						if (subdomain === 'index') {
							Object.keys(this.config.subdomains).forEach((sub) => {
								if (this.authTokens[sub] && this.authTokens[sub][strategyConfigName]) {
                    ...
                    (req, res, next, opts = {}) => {
						const { subdomain } = res.locals
						const stateFromIndexRequest = this.authTokens[subdomain] && this.authTokens[subdomain][opts.middlewareConfigName] ? this.authTokens[subdomain][opts.middlewareConfigName].state : undefined
                        if (req.query.state == req.session.state || (stateFromIndexRequest && (req.query.state === stateFromIndexRequest))) {
        ...
        const googleUserValidator =
            typeof this.config.authentication.google.googleUserValidator === 'function'
                ...
                : (profile, subdomain, authTokens) => {
                      ...
                      if (authTokens.opts.googleEmailAddress)
                          return (
                              profile !== null &&
                              profile.username.toLocaleLowerCase() ===
                                  authTokens.opts.googleEmailAddress.toLocaleLowerCase()
                          )
                      if (authTokens.opts.username)
                          return (
                              profile !== null &&
    ...
    if (this.config.authentication.github && this.config.authentication.github.githubClientID) {
        ...
        const githubUserValidator =
            typeof this.config.authentication.github.githubUserValidator === 'function'
                ...
                : (profile, subdomain, authTokens) => {
                      // console.log({ googleProfile: profile, authTokens, subdomain })
                      if (authTokens.opts.githubEmailAddress)
                          return (
                              profile !== null &&
                              profile.username.toLocaleLowerCase() ===
                                  authTokens.opts.githubEmailAddress.toLocaleLowerCase()
                          )
                      if (authTokens.opts.username)
                          return (
                              profile !== null &&
