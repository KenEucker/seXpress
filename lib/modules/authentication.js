    ...
    const configurePassportMiddleware = (
		...
		const attemptToAuthorizeForSubdomain = (req, accessToken, refreshToken, profile, done) => {
			...
			if (profilesMatched.length) {
				this.log.status(`user authenticated via third party auth`, { strategy: middlewareName, profile })
        ...
        for (const subdomain of Object.keys(this.config.subdomains)) {
			...
			if (uniqueClientStrategies.indexOf(subdomainClientID) === -1) {
				this.log.debug(`Additional client ID for auth strategy discovered`, { auth: middlewareName, subdomainClientID })
			...
			const refreshToken = (subdomain) => {
				...
				refresh.requestNewAccessToken(
					...
					(err, accessToken, refreshToken) => {
						this.log.status(
							`${middlewareName} access token has been refreshed:`, {
								refreshToken,
								middlewareName,
							}
						)
