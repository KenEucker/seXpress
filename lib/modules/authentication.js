...
function InitAuthentication(initial, authenticationOps = {}) {
	...
	*/
    ...
    if (this.config.authentication.imgur && this.config.authentication.imgur.imgurClientID) {
		...
		const imgurUserValidator = typeof this.config.authentication.imgur.imgurUserValidator === 'function' ? this.config.authentication.imgur.imgurUserValidator : (profile, subdomain, authTokens) => {
						
			if (authTokens.imgurEmailAddress) return profile !== null && profile.username === authTokens.imgurEmailAddress
			if (authTokens.username) return profile !== null && profile.username === authTokens.username
    ...
    if (this.config.authentication.reddit && this.config.authentication.reddit.redditClientID) {
		...
		const redditUserValidator = typeof this.config.authentication.reddit.redditUserValidator === 'function' ? this.config.authentication.reddit.redditUserValidator : (profile, subdomain, authTokens) => {
			console.log({ redditProfile: profile, authTokens, subdomain })
			
			if (authTokens.redditEmailAddress) return profile !== null && profile.username === authTokens.redditEmailAddress
			if (authTokens.username) return profile !== null && profile.username === authTokens.username
    ...
    if (this.config.authentication.google && this.config.authentication.google.googleClientID) {
		...
		const googleUserValidator = typeof this.config.authentication.google.googleUserValidator === 'function' ? this.config.authentication.google.googleUserValidator : (profile, subdomain, authTokens) => {
			// console.log({ googleProfile: profile, authTokens, subdomain })
			if (authTokens.googleEmailAddress) return profile !== null && profile.username === authTokens.googleEmailAddress
			if (authTokens.username) return profile !== null && profile.username === authTokens.username
    ...
    if (this.config.authentication.github && this.config.authentication.github.githubClientID) {
		...
		const githubUserValidator = typeof this.config.authentication.github.githubUserValidator === 'function' ? this.config.authentication.github.githubUserValidator : (profile, subdomain, authTokens) => {
			// console.log({ googleProfile: profile, authTokens, subdomain })
			if (authTokens.githubEmailAddress) return profile !== null && profile.username === authTokens.githubEmailAddress
			if (authTokens.username) return profile !== null && profile.username === authTokens.username
