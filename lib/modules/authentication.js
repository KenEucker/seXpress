...
function InitAuthentication(initial, authenticationOps = {}) {
    ...
    } = require('../util')(this.config.appRoot)
    ...
    const configurePassportMiddleware = (
        ...
        const convertAuthOptsToStrategyOpts = (authOpts, defaults) => {
            const _defaults = {
                ...
                userAgent: (authOpts[keys.userAgent] || '').replace('VERSION', this.config.version).replace('URL', this.getBaseUrl()),
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
			...
			console.log({middlewareConfigName, authStrategyOpts})
			...
			if (typeof customCallback === 'function') {
				...
				this.app.get(
					...
					(req, res, next) => {
						return customCallback(req, res, next, { middlewarePassport, middlewareConfigName, scope: authStrategyOpts.scope })
					},
				)
			} else {
				/// These are the only custom things for a given strategy
			...
			if (typeof customAuthCallback === 'function') {
				this.app.get(
					...
					this.requestHandler((req, res, next) => {
						return customAuthCallback(req, res, next, { successRedirect, failureRedirect, middlewareConfigName, middlewarePassport })
					}),
				)
			} else {
				this.app.get(
            ...
            if (strategyConfigName.indexOf(redditAuthPrefix) === 0) {
                configurePassportMiddleware(
					...
					(req, res, next, opts = {}) => {
						...
						return opts.middlewarePassport.authenticate(opts.middlewareConfigName, merge(opts.authStrategyOpts, {
