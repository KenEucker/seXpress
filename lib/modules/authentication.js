    ...
    const configurePassportMiddleware = (
        ...
        const setTokensForSubdomain = (subdomain, accessToken, refreshToken, profile) => {
            if (profileMatcher(profile, subdomain)) {
                ...
                this.authTokens[subdomain][middlewareName][keys.refreshToken] = [keys.refreshToken] || refreshToken
                this.authTokens[subdomain][middlewareName][keys.profile] = currentTokens[keys.profile] || profile
        ...
        const attemptToAuthorizeForSubdomain = (req, accessToken, refreshToken, profile, done) => {
            ...
            const profilesMatched = setTokens(accessToken, refreshToken, profile, subdomain !== 'index' ? subdomain : null)
            ...
            const strategyOpts = util.merge(
                {
                    ...
                    callbackURL:
                        authOpts[keys.callbackURL] || `/auth/${middlewareName}/callback`,
