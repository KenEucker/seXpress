...
module.exports = function (authenticationOps = {}) {
	...
	*/
    this.config.authentication = this.getCoreOpts(moduleName, authenticationOps, false)
    ...
    const configurePassportMiddleware = (
        ...
        keys = keys || {
			...
			clientAuthorization: `${middlewareName}ClientAuthorization`,
			clientProfile: `${middlewareName}Profile`,
        }
        ...
        const subdomainGetTokenRequest = (subdomain, req, res) => {
			const response = {}
			/// Get the current authorization value or the defaults used during initialization
			const middlewareOpts = util.merge(this.authTokens[subdomain][middlewareName], this.authTokens[subdomain][middlewareName].opts)

            response[keys.clientID] = middlewareOpts.clientID
            response[keys.clientProfile] = middlewareOpts.clientProfile
			
            if (this.isValidRequestOrigin(req)) {
				response[keys.clientAuthorization] = middlewareOpts.clientAuthorization
				response[keys.clientRefreshToken] = middlewareOpts.clientRefreshToken
                response[keys.clientAccessToken] = middlewareOpts.clientAccessToken
                response[keys.clientProfile] = middlewareOpts.clientProfile
            }
        ...
        const convertAuthOptsToStrategyOpts = (authOpts, defaults = {}) => {
            const strategyOpts = util.merge(
                {
                    passReqToCallback: true,
                    scope: authOpts.scope || ['email'],
                    clientID: authOpts[keys.clientID],
                    clientSecret: authOpts[keys.clientSecret],
                    callbackURL:
                        authOpts[keys.clientCallbackURL] || `/auth/${middlewareName}/callback`,
                    clientAccessToken: authOpts[keys.clientAccessToken],
                    clientRefreshToken: authOpts[keys.clientRefreshToken],
                },
                defaults,
            )
        ...
        const defaultStrategyOpts = convertAuthOptsToStrategyOpts(securityOpts[middlewareName])
        ...
        for (const clientID of Object.keys(uniqueClientStrategies)) {
            ...
            const firstSubdomainSet =
                typeof validSubdomains === 'string' ? validSubdomains : validSubdomains[0]
            ...
            const interceptSubdomains =
                validSubdomains.indexOf('default') !== -1 ? undefined : interceptSubdomains
    ...
    this.log.info(
        `🗝  authorization providers have been initialized`,
