    ...
    const configurePassportMiddleware = (
        ...
        const attemptToAuthorizeForSubdomains = (req, accessToken, refreshToken, profile, done) => {
			...
			if (subdomain === 'index') {
            	...
            	for (const sub of subdomainsToSet) {
					const subdomainCallbackUrl = `${this.getBaseUrl(undefined, undefined, sub)}${this.authTokens[sub][middlewareConfigName].opts.callbackURL}`
					...
					req.uest({
						...
						console.log("HELLLOOOOO", {sub, middlewareConfigName})
						if (er) {
