	...
	*/
    ...
    if (this.config.defaults.googleClientID) {
        ...
        const setGoogleTokens = (accessToken, refreshToken, profile) => {
            for (const subdomain of subdomains) {
                authTokens[subdomain].google.googleAccessToken = accessToken
                authTokens[subdomain].google.googleRefreshToken =
                    authTokens[subdomain].google.googleRefreshToken || refreshToken
                authTokens[subdomain].google.googleProfile =
                    authTokens[subdomain].google.googleProfile || profile
                self.log(
                    `google authentication information for subdomain: subdomain`,
                    authTokens[subdomain].google,
                )
            }
        }
        ...
        const googleStrategy = new GoogleStrategy(
			{
				clientID: this.config.defaults.googleClientID,
				clientSecret: this.config.defaults.googleClientSecret,
				callbackURL: this.config.defaults.googleCallbackURL,
				passReqToCallback: true,
			},
        ...
        const refreshGoogleTokens = () => {
            const theRefreshTokenToUse = authTokens.default.google.googleRefreshToken
            self.log.status(
		...
		setInterval(refreshGoogleTokens, googleRefreshFrequency)
		this.app.post('/auth/google/getToken', (req, res) => {
            ...
            if (this.isValidRequestOrigin(req)) {
                response.googleRefreshToken = authTokens[subdomain].google.googleRefreshToken
                response.googleAccessToken = authTokens[subdomain].google.googleAccessToken
                response.googleProfile = authTokens[subdomain].google.googleProfile
            }
