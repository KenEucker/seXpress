...
function InitConfig(initial, infoOpts = {}) {
    ...
    if (this.config.ui.enabled) {
		...
		const {getFromQueryOrPathOrBody, getSubdomainPrefix} = require('../util')(this.config.appRoot)
		const self = this
		...
		const jsonEditorSvgFileNamePath = join(jsonEditorNodeModulesPath, 'img', jsonEditorSvgFileName)
		...
		if (existsSync(jsonEditorJsFilePath) && existsSync(jsonEditorCssFilePath)) {
			...
			const modifiedJsonEditorCssFile = jsonEditorCssFile.toString().replace(svgFileRegex, `/public/img/${jsonEditorSvgFileName}`)
			...
			if (existsSync(jsonEditorSvgFileNamePath)) {
				copyFileSync(jsonEditorSvgFileNamePath, join(publicImgDestinationPath, jsonEditorSvgFileName))
			}
		}
        ...
        this.route(
			'/:subdomain?',
            function configIndexHandler(req, res, next) {
				const subdomain = getSubdomainPrefix(self.config, req)
				if (subdomain === 'config' && req.isAuthenticated()) {
