...
function InitConfig(initial, infoOpts = {}) {
    ...
    if (this.config.ui.enabled) {
        ...
        const { getFromQueryOrPathOrBody, getSubdomainPrefix } = require('../util')(
            this.config.appRoot,
        )
        const self = this
        ...
        const removeSensitiveObjects = (config, disallowedKeys = []) => {
            ...
            keys.forEach((key) => {
                ...
                if (
                    ...
                    out[key] =
                        keyType === 'object' ? removeSensitiveObjects(config[key]) : config[key]
        ...
        this.route(
            ...
            function configIndexHandler(req, res, next) {
                ...
                if (subdomain === 'config' && req.isAuthenticated()) {
                    ...
                    return self.renderTemplateOrView('config/index', { configData }, res)
