...
async function InitCompilation(initial, compilationOpts) {
    ...
    if (this.config.compilation.enabled) {
        return new Promise(async (resolve, reject) => {
			...
			compilerOptions.outDir = compilerOptions.outDir ? join(this.config.folders.appFolder, compilerOptions.outDir) : undefined
			compilerOptions.rootDir = compilerOptions.outDir ? join(this.config.folders.appFolder, compilerOptions.rootDir) : undefined
