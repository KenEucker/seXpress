...
module.exports = function (apiOpts = {}) {
	if (this.config.api.enabled) {
		...
		apiOpts = util.merge(apiOpts, util.merge({
			docsEndpoint: this.config.api.docsEndpoint || '/api/docs',
			subdomains: Object.keys(this.config.subdomains),
			ignoreSubdomains: ['index']
		}, this.config.api))
		
		const ignoreSubdomains = Object.keys(this.core).filter((m) => apiOpts.ignoreSubdomains.indexOf(m) === -1)
		const subdomains = apiOpts.subdomains.filter(
