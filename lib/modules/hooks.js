    ...
    if (this.config.hooks.enabled) {
        ...
        const getHookInfo = (id) => {
            ...
            if (existingHookIndex !== -1) {
                return {
                    registered: true,
                    type: typeof this.hooks[id],
					count: this.hooks[id].length,
                }
            }
        ...
        const sendHook = async (id, recieve = true) => {
            if (reservedHookMethods.indexOf(id) !== -1) {
        ...
        this.log.info(`${moduleEmoji} Applicaition hooks have been enabled`, this.config.notifications)
    }
}
