class MongodbMiddleware {
    ...
    async init(middlewares) {
        ...
        await this.compose.upAll(this.opts.composeOpts).then(
            () => {
                ...
                console.log('docker container for mongodb/mongodb-express has started')
            },
            (err) => {
                this.opts.server = false
                console.log('docker container for mongodb/mongodb-express failed to start', err)
            },
        )
    }
    ...
    async close() {
        if (this.opts.server && !this.opts.persist) {
            console.log('closing mongodb', this.opts.composeOpts)
            try {
