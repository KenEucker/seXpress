...
class util {
	constructor(AppRoot = require('app-root-path')) {
		this.appRoot = AppRoot
		this.log = this.wrapLogger(console.log)
		...
		this.setInterval = require('safe-timers').setInterval
	}

	wrapLogger(_logger) {
		_logger.error = _logger.error || _logger

		const error = (message, obj = '') => _logger.error(singleton.instance.chalk.red(message), obj)
		const info = (message, obj = '') => _logger(singleton.instance.chalk.blueBright(message), obj)
		...
		const debug = (message, obj = '') => _logger(singleton.instance.chalk.blueBright(message), obj)
	...
	getSubdomainPrefix(opts, req, returnAlias = false) {
		...
		return returnAlias ? baseSubdomain : singleton.instance.getSubdomainFromAlias(opts, baseSubdomain)
	}
	...
	getViews(opts, viewsFolder) {
		...
		singleton.instance.fs.readdirSync(viewsFolder).forEach((filename) => {
			if (singleton.instance.path.extname(filename) !== `.${opts.rendering.overrideViewEngine}`) return
		...
		const _getControllers = (folder, importAll) => {
			...
			controllersFiles.forEach((filename) => {
				...
				if (singleton.instance.fs.statSync(file).isDirectory()) {
					if (filename !== 'views' && importAll) _getControllers(singleton.instance.path.join(folder, filename))
					return
				}
	...
	getHostUri(opts, host, protocol = '', append = '/api', fullHost = true) {
		return `${singleton.instance.getHostBaseUrl(opts, host, protocol, undefined, fullHost)}${append}`
	}
	...
	getServers (
		...
		if (subdomain === 'api' || subdomain == opts.routing.indexControllerName) {
			...
			Object.keys(opts.subdomains).forEach((sub) => {
				...
				const url = singleton.instance.getHostUri(opts, `${apiSubPrefix}${apiHost}`, protocol, `/${sub}`, true)
	...
	findViewFile(opts, view, engine) {
		...
		const viewFileIndex = singleton.instance.path.join(opts.folders.viewsFolder, `${view}/index.${engine}`)
	...
	findTemplateFile(opts, engine) {
		const viewFile = singleton.instance.path.join(opts.folders.templatesFolder, `${view}.${engine}`)
...
class UtilFactory {
	constructor() {
		...
		console.log({instance: singleton, namespace})
