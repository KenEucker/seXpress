    ...
    close() {
		return new Promise((resolve, reject) => {
			if (this.opts.server && !this.opts.persist) {
				console.log('wordpress closing')
				const downPromise = this.compose.down(this.opts.composeOpts)
				console.log({downPromise, then: downPromise.then})
				return downPromise.then(resolve)
					.then((e) => {
						console.log('wordpress closed', e)
						resolve()
					})
					.catch((err) => {
						console.log('wordpress failed to close due to error', err) ; reject()
					})
					.finally(() => {
						console.log("FINALLY")
					})
