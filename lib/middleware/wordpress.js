class WordpressMiddleware {
    ...
    async close() {
        if (this.opts.server && !this.opts.persist) {
			console.log('RYING')
			try {
            await this.compose.down({...this.opts.composeOpts, commandOptions: ['--volumes']}).then(
                () => {
                    console.log('wordpress docker container stopped')
                },
                (err) => {
                    console.log('could not stop wordpress docker image', {
                        image: this.opts.wordpressDockerImage,
                        err,
                    })
                },
			)
