...
const _getPublicConfigurationValues = (opts, subdomain, host, overrides) => {
    ...
    Object.keys(opts.subdomains).forEach((subdomainName) => {
		...
		if (subdomain === subdomainName) {
			publicConfig.page = getValuesFromObjectOrDefault(undefined, pageData, overrides, undefined, true)
		}
		console.log({subs: opts.subdomains, subdomainName, subdomainInformation})
...
const _getSubdomainOpts = (opts, req) => {
    const subdomain = _getSubdomainPrefix(opts, req, true)
    let subdomainConfig = {}
...
const _isValidRequestOrigin = (opts, req) => {
...
class Sexpress {
    ...
    getSubdomainOpts(req) {
        return _getSubdomainOpts(this.config, req)
    }
    ...
    isValidRequestOrigin(req) {
        return _isValidRequestOrigin(this.config, req)
	}
    ...
    templating(supportRendering = true) {
        ...
        if (!!this.config.subdomains) {
            ...
            if (this.config.headless) {
                ...
                headHandler = (subdomain, req, res, host, next) => {
            ...
            this.routeSubdomainRequest('/', headHandler)
    ...
    authentication() {
        ...
        if (this.config.defaults.imgurClientID) {
            ...
            this.app.post('/auth/imgur/getToken', (req, res) => {
                ...
                if (_isValidRequestOrigin(this.config, req)) {
