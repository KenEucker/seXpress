...
class Sexpress {
    ...
    requestHandler(handler, subdomains, skipLogging = false) {
        return (req, res, next) => {
            ...
            const sudomainIsApi = subdomain === 'api'
            ...
            /// Add the request information to the locals
    ...
    renderTemplate(template, data, res, engine) {
        ...
        if (this.config.rendering.enabled) {
			const viewTemplateFound = fs.existsSync(viewTemplate) ? viewTemplate : (fs.existsSync(viewTemplateIndex) ? viewTemplateIndex : null)

			if (viewTemplateFound) {
				this.log.debug('rendering template', { data, viewTemplateFound, locals: res.locals })
		...
		const htmlFallbackTemplateFound = fs.existsSync(htmlFallbackTemplate) ? htmlFallbackTemplate : (fs.existsSync(htmlFallbackTemplateIndex) ? htmlFallbackTemplateIndex : null)
