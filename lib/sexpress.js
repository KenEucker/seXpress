...
class Sexpress {
    makeArt() {
        return new Promise((r) => {
            ;(() => {
                ...
                const packageJsonPath = path.join(dirname(require.main.filename), 'package.json')
    ...
    constructor(opts = {}) {
        ...
        /// Catch the exit of the process and run the stop method
        exitHook(this.stop.bind(this))
    ...
    init(debug) {
        return this.util.promiseMe(async (done) => {
            ...
            this.setConfiguration({
                ...this.config,
                ...this.config.onConfigurationLoad(this.config),
            })
            ...
            await this.initMiddlewares(this.config.middlewares)
            ...
            if (this.config.onLoad) {
                await this.config.onLoad(this.config)
            }
    ...
    async initMiddlewares(middlewares = {}) {
        this.middlewares = {}
        ...
        await middlewaresNames.forEach(async (middleware) => {
    ...
    setConfiguration(config) {
        ...
        /// TODO: put all ssl specific code into the ssl module
        ...
        const defaultAuthorName = this.config.name.replace(' ', '_').toLocaleLowerCase()
        const defaultAuthorEmail = `${this.config.name.replace(' ', '.')}@${this.config.host}`
        const authorName =
            !!this.config.author && typeof this.config.author === 'object'
                ? this.config.author.name
                : typeof this.config.author === 'string' && this.config.author.lenght
                ? this.config.author
                : defaultAuthorName
        const authorEmail =
            !!this.config.author && typeof this.config.author === 'object'
                ? this.config.author.email
                : typeof this.config.author === 'string'
        ...
        this.config.authentication.secret = this.config.authentication.secret || authorName
    ...
    registerController(controller, root = '') {
        ...
        if (this.config.hooks?.enabled && controller.hooks?.length) {
        ...
        return this.util.promiseMe(async (done) => {
            ...
            if (modules) {
                for (let i = 0; i < order.length; ++i) {
                    ...
                    const moduleFunction = modules[module]
                    const moduleDefaults = moduleFunction.defaults
                    ...
                    const moduleInitPromise = moduleFunction.bind(this, moduleDefaults)()

                    /// Wait for the initialization of the module
                    ...
                    /// Wire up the module's hooks to the configured remotes
                    const moduleWire = this.config[module]?.wired
                    if (moduleWire && moduleFunction.hooks?.length) {
                        if (Array.isArray(moduleWire)) {
                            Object.values(moduleFunction.hooks).forEach((hook) => {
                                const wire = moduleWire[hook]
    ...
    hook(endpoint, payload, register = true) {
        if (this.hooks) {
            const endpointInfo = this.hooks.getHookInfo(endpoint)
            const payloadType = typeof payload

            if (payloadType === 'function') {
                /// If the payload is a function, either
                if (!endpointInfo.registered || (endpointInfo.registered && register)) {
                    /// set/overwrite the registered hook with the payload
                    this.hooks.addHook(endpoint, payload)
                } else {
                    /// pass the registered hook to the payload to be called with custom opts
                    return payload(this.hooks[endpoint])
                }
            } else if (payloadType === 'object' || payloadType === 'string') {
                /// If the payload is a function or object, either
                if (endpointInfo.registered && endpointInfo.type === 'function') {
                    /// call the registered hook with the payload
                    this.hooks.triggerHook(endpoint, payload)
                } else {
                    /// set/overwrite the registered hook with the payload
                    this.hooks.addHook(endpoint, payload)
                }
                ...
                this.log.error(
                    `hook [${endpoint}] cannot be registered or invoked with the provided payload`,
    ...
    getPublicData(subdomain, host, overrides = {}, res, uncached = false) {
        ...
        return this.config.onPageDataRequest(getPublicDataResponse, this.config, subdomain)
    }
    ...
    run(onStart = () => {}) {
        ...
        return Promise.allSettled(this._initPromise).then(
    ...
    async stop(end = () => {}) {
		...
		const middlewareClosingPromises = []
        await Object.keys(this.middlewares).forEach(async (middleware) => {
            if (this.middlewares[middleware].close) {
                // console.log(`closing middleware: ${middleware}`)
                middlewareClosingPromises.push(this.middlewares[middleware].close())
                // console.log(`middleware closed: ${middleware}`)
            }
        })
		await Promise.allSettled(middlewareClosingPromises)

        // console.log(`all middlewares closed`)

        if (this.config.preserve) {
            const preservedConfig = {}
            /// config.js files are loaded after config.json files, so zzz should always come last?
            const preservedConfigFilePath = path.join(
                this.config.folders.configFolder,
                `zzz.config.json`,
            )
            const preserveOptions =
                Array.isArray(this.config.preserve) ? this.config.preserve : ['authentication']
