    ...
    route(endpoint, response, methods = 'get', secure = false) {
        ...
        const isAuthenticated = (req, res, next) => {
            if (!req.isAuthenticated()) {
				const activeAuthStrategies = this.config.security.schemes ? this.config.security.schemes.map((s) => s.name) : ['basic', 'local']

                /// Try all of the authentication methods
                return passport.authenticate(
                    activeAuthStrategies,
                    (err, user) => {
						this.log.debug('exhausted authenticators', { err, user })
    ...
    getSwaggerSpec(opts, overrides) {
        ...
        if (opts.security.enabled) {
            ...
            if (opts.security.schemes) {
                const useTheseSchemesOnly = opts.security.schemes.map((s) => s.name)
