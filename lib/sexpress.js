...
class Sexpress {
    ...
    registerController(controller, root = '') {
        ...
        let viewEngine = this.getDefaultViewEngine(controller.engine)
    ...
    getAvilableSSOProviders(subdomain) {
        ...
        let securtiyOpts = this.config.subdomains[subdomain]
        /// TODO: change this to an option for login subdomains?
        if (!securtiyOpts) {
            securtiyOpts = this.config.security
        }
    ...
    getCoreOpts(moduleName = '', overrides = {}, defaults = {}) {
        const moduleOpts = util.merge3(defaults, this.config[moduleName], overrides)
	...
	getDefaultViewEngine(override) {
		...
		if (this.config.rendering.overrideViewEngine) {
			if (typeof this.config.rendering.overrideViewEngine === "object" || typeof this.config.rendering.overrideViewEngine === "array") {
    ...
    renderViewOrTemplate(view, data, res) {
		...
		let foundView = false
		if (typeof engine === 'undefined' && typeof this.config.rendering.overrideViewEngine === 'array' || typeof this.config.rendering.overrideViewEngine === 'array') {
			console.log({YES: this.config.rendering.overrideViewEngine})
			this.config.rendering.overrideViewEngine.forEach((engine) => { foundView = findViewFile(engine) ; if (foundView) return false})
		} else {
    ...
    renderTemplate(template, data, res, engine) {
        ...
        if (typeof engine === 'array') {
            ...
            while (
                index < engine.length &&
                !(success = renderTemplate(template, data, res, engine[index]))
            ) {
                index++
            }
