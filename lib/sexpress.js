...
const debug = !!config.debug
    ? config.debug
	: process.argv.reduce((out, arg) => (out = arg.indexOf('debug=') === 0 ? arg.substring("debug=".length) : out), false)

/// Configuration is first initialized here, upon import
config = util.merge(
    config,
    debug && fs.existsSync(debugFilename)
        ? util.merge(require(debugFilename), {
              host: 'localhost',
              port: 8080,
              debug: true,
          })
...
class Sexpress {
    ...
    setConfiguration(config) {
        ...
        this.config.author = {
            ...
            email: this.config.author
                ? util.extractEmails(this.config.author)[0]
        ...
        methods.forEach((method) => {
            ...
            if (routeIsApiEndpoint && !forceInsecure) {
                ...
                if (secure) {
					...
					this.app[method](endpoint, this.requestHandler(undefined, subdomains), this.isAuthenticatedHandler(response))
				} else {
			...
			if (!this._customControllerRoutePrefix || !this._customControllerRoutePrefix.length) {
				this.log.debug(`✨  <${method}> -> ${endpoint}${secure ? " *" : ""}`, [functionName])
			}
        })
    }
    ...
    requestHandler(handler, restrictedSubdomains = [], skipLogging = false) {
        return (req, res, next) => {
            ...
            if (restrictedSubdomains.length && !this.isValidSubdomain(subdomain, restrictedSubdomains, this.coreSubdomains)) {
				if (!this.config.templating.headless || subdomain === this.config.routing.indexControllerName) {
                    return next()
                }
                ...
                this.log.error(`🛑 request invalid for subdomain [${subdomain}]`, {
                    ...
                    subdomain,
                    subdomains: restrictedSubdomains,
                    url: req.url,
                    host: req.hostname,
                ...
                this.log.status(
                    ...
                    }] request:${ip} (${subdomain} > ${url})`,
                    {
                        handler: handler ? (handler.name
                            ? handler.name
                            : util.getFunctionName(handler) || '*()') : 'passthrough',
                        url,
                        subdomain,
                        ip,
			...
			/// TODO: change the callback structure back to expres defaults of (req, res, next) and have all references to subdomain and host come from res.locals
    ...
    getAvilableSSOProviders(subdomain) {
        ...
        const subdomainOpts = this.config.subdomains[subdomain]
        if (subdomainOpts) {
            let securtiyOpts = subdomainOpts.authentication
    ...
    isValidSubdomain(subdomain, validSubdomains = [], ignoreSubdomains = []) {
        ...
        const subdomainIsConfiguredCoreModule = isConfiguredSubdomain && this.coreSubdomains.indexOf(subdomain) !== -1
		const isNotRestricted = validSubdomains.indexOf(subdomain) !== -1 || subdomainIsConfiguredCoreModule

		if (!isNotRestricted) console.log({subdomain, validSubdomains, subdomainIsConfiguredCoreModule})
        ...
        return Promise.all(this._initPromise).then(
            async function RunSexpress() {
                ...
                if (this.config.ssl.enabled) {
                    ...
                    if (httpsServer) {
                        httpsServer
                            .listen(this.config.ssl.port, () => {
                                this.log.status(
                                    `\n💃 *{${this.config.name}}* @ ${this.config.protocol}://${this.config.host}:${this.config.ssl.port} 💃 \n\n`,
                                )
                            })
