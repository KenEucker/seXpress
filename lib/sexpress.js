...
const _getServers = (opts, host, protocol, subdomain, forceSecure = false) => {
    ...
    if (subdomain !== opts.indexControllerName) {
        ...
        Object.keys(opts.subdomains).forEach((subdomain) => {
            if (subdomain === opts.indexControllerName) return

            servers.push({
                url: _getHostUri(opts, `${subdomain}.${host}`, forceSecure ? 'https' : protocol),
            })
        })
	}
...
class Sexpress {
        ...
        if (fs.existsSync(this.config.controllersFolder)) {
            ...
            this.route('/api/swagger.json', (subdomain, req, res, host) => {
                const swaggerSpec = this.getSwaggerSpec(this.config, {
                    servers: _getServers(this.config, host, req.protocol, subdomain, this.config.apiSecurity && host.indexOf('localhost') === -1),
                })
    ...
    getSwaggerSpec(opts, overrides) {
        ...
        if (opts.apiSecurity) {
            ...
            if (opts.apiSecurityAll) {
                openApiDefinition.security = Object.keys(securitySchemas).reduce((o, s) => {
                    o[s] = []
                    return o
                }, {})
            }
    ...
    initApiSecurity(strategies = [], jwtOpts = {}) {
        const allStrategies = ['basic', 'bearer']
