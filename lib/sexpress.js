...
const _getPublicConfig = (opts, subdomain, host, overrides) => {
    ...
    Object.keys(opts.subdomains).forEach((subdomainName) => {
        ...
        const pageData = getValuesFromObjectOrDefault(opts.publicFields, subdomainInformation, opts)
...
const _getControllers = (opts, controllersFolder, ignore) => {
    ...
    const getControllers = (folder, importAll) => {
        ...
        controllersFiles.forEach((filename) => {
            ...
            const controllerName = _getControllerNameFromFilePath(folder)

            /// Import all folders under this folder
            if (fs.statSync(file).isDirectory()) {
                if (filename !== 'views' && importAll) getControllers(path.join(folder, filename))
                return
            }
...
const _getServers = (opts, host, protocol, subdomain, forceSecure = false) => {
    const servers = []

    if (subdomain !== opts.indexControllerName) {
        servers.push({
            url: _getHostUri(opts	, host, protocol),
        })
    } else {
        Object.keys(opts.subdomains).forEach((subdomain) => {
            ...
            servers.push({
                url: _getHostUri(opts, `${sub}${host}`, forceSecure ? 'https' : protocol),
            })
        })
    }
...
class Sexpress {
    ...
    /// Add the api routes at hostname/api
    _apiModule() {
        if (fs.existsSync(this.config.controllersFolder)) {
            ...
            if (fs.existsSync(openApiDefinitionFile)) {
                ...
                this.config.openApiDefinition = require(openApiDefinitionFile)
            }
    ...
    /// Add OpenApi defined documentation at hostname/api/docs
    _docsModule() {
        if (this.config.documentAPI) {
            ...
            this.route(apiDocsEndpoint, (subdomain, req, res, host, next) => {
                const swaggerSpec = this.getSwaggerSpec(this.config, {
                    servers: _getServers(
                        this.config,
                        host,
    ...
    _routingModule(controllersFolder) {
        controllersFolder = controllersFolder || this.config.controllersFolder

        if (fs.existsSync(controllersFolder)) {
            const controllers = _getControllers(this.config, controllersFolder)
            ...
            controllers.forEach((file) => {
                ...
                this.registerController(
                    controller,
                    isIndexController ? '' : _getControllerNameFromFilePath(file),
                )
            })
        }
    }

    /// Adds basic html and sexy rendering support
    ...
    _authenticationModule() {
		...
		const securityIsAllOrNothing = !(typeof this.config.security === 'object')
		if (this.config.security && (securityIsAllOrNothing ? this.config.security.enabled : this.config.security)) {
			this.config.security = securityIsAllOrNothing ? { enabled: true } : this.config.security

			/// Add the login subdomain controller
    ...
    /// Generates a sexpress application out of a single json object
    _generateModule(applicationDefinition) {
    ...
    /// Reduces this sexpress application to a single json object
    _export() {
        // const apiDocsTemplateDestination = path.resolve(this.config.templatesFolder, 'docs')
        /// TODO: send to temporary folder to archive into a .zip file that can be downloaded from the api/docs path alongside the swagger.json
        // const swaggerServerCodegen = require('swagger-node-codegen')
    ...
    setConfiguration(config) {
        ...
        this.config.viewsFolder = this.config.viewsFolder
            ...
            : getRootPath(path.join('controllers', 'views'))
		...
		this.config.security = typeof this.config.security === 'object' ? this.config.security : { enabled: this.config.security }
    ...
    getSwaggerSpec(opts, overrides) {
        ...
        if (opts.security.enabled) {
            ...
            if (opts.security.schemes) {
                Object.keys(securitySchemes).forEach((strategy) => {
                    if (opts.security.schemes.indexOf(strategy) === -1) delete securitySchemes[strategy]
                })
            }
        ...
        const jsDocOpts = {
            swaggerDefinition,
            apis: _getControllers(opts, undefined, opts.privateApis),
        }
    ...
    initApiSecurity(securityOpts = {}, jwtOpts = {}) {
        ...
        schemes.forEach((scheme) => {
			...
			const schemeName = allOrNothingScheme ? scheme : scheme.name

			let logMessage = `setting ${schemeName} security strategy`
            switch (schemeName) {
                case 'basic':
					const defaultUserField = 'username'
					const defaultPassField = 'password'
					const usernameField = allOrNothingScheme ? defaultUserField : (scheme.usernameField || defaultUserField)
					const passwordField = allOrNothingScheme ? defaultPassField : (scheme.passwordField || defaultPassField)

					if (!allOrNothingScheme) scheme.validateUser = scheme.validateUser || ((u, p, d) => d("No Validation Method set"))
                    ...
                    passport.use(
                        new LocalStrategy({
							...
							return scheme.validateUser(username, password, done)
                        }),
                    )
                    break
	...
	renderViewOrTemplate(view, data, res) {
		const viewFile = path.join(this.config.viewsFolder, `${view}.${this.config.overrideViewEngine}`)
