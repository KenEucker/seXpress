...
class Sexpress {
    ...
    requestHandler(handler, subdomains, skipLogging = false) {
        return (req, res, next) => {
            ...
            if (!this.isValidSubdomain(subdomain, subdomains)) {
                ...
                console.log('redirecting requestHandler')
    ...
    getCoreOpts(moduleName = '', overrides = {}, initial) {
		...
		defaults.enabled = typeof defaults.enabled !== 'undefined' ? defaults.enabled : (typeof initial === 'undefined' ? true : !!initial)
        ...
        this.log.debug(`initialized core module options for [${moduleName}]`, {
            initial,
            defaults,
            fromConfig,
            overrides,
            ...
            if (!req.isAuthenticated()) {
                ...
                return passport.authenticate(activeAuthStrategies, (err, user) => {
                    ...
                    if (req.method === 'GET') {
                        ...
                        console.log('redirecting isAuthenticatedHandler')
    ...
    isValidSubdomain(subdomain, validSubdomains, ignoreSubdomains = ['api', 'login']) {
        ...
        const reject = hasRestrictedSubdomains ? isNotRestricted : subdomainIsCoreModule && isConfiguredSubdomain
    ...
    renderTemplateOrView(template, data, res) {
		...
		const templateFile = path.join(this.config.folders.templatesFolder, `${template}.${engine}`)
    ...
    renderTemplate(template, data, res, engine) {
		...
		const fullPathIsInTemplateName = template.indexOf(this.config.folders.templatesFolder) !== -1
        ...
        const pageTemplate = fullPathIsInTemplateName ? template : path.join(this.config.folders.templatesFolder, template, 'index')
