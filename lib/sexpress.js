...
class Sexpress {
            ...
            if (!this.config.middlewares) {
                ...
                middlewareFiles.forEach((middleware) => {
                    ...
                    if (middlewareName !== 'index') {
                        this.config.middlewares[middlewareName] = true
                    }
                })
            }
    ...
    setConfiguration(config) {
        ...
        this.coreSubdomains = this.config.coreSubdomains || ['api', 'config', 'login']
        ...
        return (req, res, next) => {
            ...
            if (
                ...
                this.log.error(`🛑 request invalid for subdomain [${subdomain}]`, {
                    handler,
                    handlerName: this.util.getFunctionName(handler),
                    subdomain,
                ...
                this.log.status(
                    ...
                    }] request:${ip} (${subdomain} > ${url})`,
                    {
                        handler: handler
                            ? handler.name
                                ? handler.name
                                : this.util.getFunctionName(handler) || '*()'
                            : 'passthrough',
                        url,
                        subdomain,
                        ip,
    ...
    getAvilableSSOProviders(subdomain) {
        ...
        Object.keys(this.config.authentication).forEach((authenticationName) => {
            const subdomainAuthOpts = subdomain !== 'index' && this.authTokens[subdomain] ? this.authTokens[subdomain] : this.authTokens.default
            if (
            ...
            const publicConfig = {
                ...
                api:
                    this.config.compilation && this.config.compilation.enabled
                        ? this.config.compilation.opts && this.config.compilation.opts.output.filename
	...
	getCookies(req = {}) {
		...
		console.log({reqCookie, reqCookies, sessionCookie, signedSessionCookie, headerCookie, headerCookies})
	...
	isAuthenticated(req = {}) {
		...
		if (!requestIsAuthenticated && req.passport) {
			...
			console.log({requestIsAuthenticated, cookiesSet, passport: req.passport, isAuthenticatedMethoExists: req.isAuthenticated})
			// console.trace()
	...
	authenticate(strategies, subdomain, opts) {
		...
		if (this.authTokens[subdomain] && this.authTokens[subdomain][strategies]) {
			if (opts) return this.authTokens[subdomain][strategies].passport.authenticate(strategies, opts)
			...
			if (opts) return this.authTokens.default[strategies].passport.authenticate(strategies, opts)
