...
class Sexpress {
				...
				if (sslFileExtension === "pem") {
					switch (sslFileName) {
						case 'cert':
							...
							ssl.certificateFilename = sslFile
							break
						case 'key':
							ssl.hasCertificateKey = true
							ssl.certificateKeyFilename = sslFile
							break
						case 'passphrase':
							ssl.passphrase = require(ssFile)
							break
					}
				}
			})
	...
	run(
		...
		if (!this.config.noSSL && !!this.config.ssl && !!this.config.ssl.passphrase) {
