...
class Sexpress {
    ...
    setConfiguration(config) {
		...
		this.config.authentication.secret = this.config.authentication.secret || this.config.name.replace(' ','_').toLocaleLowerCase()
    ...
    route(endpoint, response, methods = 'get', secure, subdomains) {
		...
		methods.forEach((method) => {
            ...
            if (routeIsApiEndpoint &&!forceInsecure) {
				...
				endpoint = `${this._customControllerRoutePrefix ? `/${this._customControllerRoutePrefix}` : ''}${endpoint}`
    ...
    getPublicData(subdomain, host, overrides = {}, uncached = false) {
