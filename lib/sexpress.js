...
const getPublicConfigurationValues = (
	...
	publicConfig.subdomains = Object.values(
		...
		const pageData = getValuesFromObjectOrDefault(opts.publicConfigFields, subdomainInformation, opts)
...
class Sexpress {
	...
	setConfiguration(config) {
		...
		this.config.publicFolder = this.config.publicFolder ? this.config.publicFolder : getRootPath("public")
		this.config.contentFolder = this.config.contentFolder ? this.config.contentFolder : getRootPath([
			"public",
			"content",
		])
		this.config.sslFolder = this.config.sslFolder ? this.config.sslFolder : getRootPath([
			"config",
			"ssl",
		])
		this.config.templatesFolder = this.config.templatesFolder ? this.config.templatesFolder : getRootPath("templates")
		this.config.controllerFolder = this.config.controllerFolder ? this.config.controllerFolder : getRootPath("controllers")
		this.config.viewsFolder = this.config.viewsFolder ? this.config.viewsFolder : getRootPath("views")
	...
	init() {
		...
		this.app.use(
			session({
				secret: this.config.session?.secret ? this.config.session.secret : this.config.appName,
				resave: this.config.session?.resave ? this.config.session.resave : false,
				saveUninitialized: this.config.session?.saveUninitialized ? this.config.session.saveUninitialized : false,
			})
		)
		this.app.use(passport.initialize(this.config.passport ? this.config.passport : {}))
		this.app.use(passport.session(this.config.passport?.session ? this.config.passport.session : {}))
	...
	addController(controller, name = '') {
		...
		if (!name) {
			controller.useRootPath = typeof controller.useRootPath !== 'undefined' ? controller.useRootPath : true
		}

		this.log.info(`loading controller: ${name ? name : 'default'}`)

		let handler, method, postfix = '', pathMessage
		...
		for (const key in controller) {
			...
			if (controller.before) {
				applet[method](
					url,
					this.wrapRequestHandler(controller.before),
					handler
				)
			} else {
	...
	security() {
		this.app.all("/*", this.wrapRequestHandler((subdomain, req, res, host, next) => {
			const url = req.url
	...
	routers() {
		const importControllerFolder = (folder, importAll = false) => {
			...
			controllers.forEach((filename) => {
				...
				this.addController(controller, folder.replace(this.config.controllerFolder, '').length > 1 ? folder.substring(folder.lastIndexOf('/') + 1) : '')
			})
		}
					...
					if (!!this.config.subdomains[subdomain]) {
						...
						if (!!subdomainTemplate) {
							...
							if (fs.existsSync(subdomainTemplatePath)) {
								this.log.debug(`configuring static path for subdomain: ${subdomain}`, subdomainTemplatePath)
	...
	ssl(serverOpts) {
		try {
			...
			this.log.info(`configuring SSL using certificate information on port:${this.config.sslport}`, serverOpts)
