...
class Sexpress {
    ...
    loadModules(modules, order) {
        ...
        if (modules) {
            for (let i = 0; i < order.length; ++i) {
				...
				this.log.debug(`Loading module`, { name: modules[module].module, description: modules[module].description })
    ...
    route(endpoint, response, methods = 'get', secure = false) {
        ...
        const isAuthenticated = (req, res, next) => {
            if (!req.isAuthenticated()) {
				const activeAuthStrategies = this.config.security.schemes ? this.config.security.schemes.map((s) => s.name || s) : ['basic', 'local']

                /// Try all of the authentication methods
                return passport.authenticate(
                    activeAuthStrategies,
                    (err, user) => {
						this.log.debug('exhausted authenticators', { err, user, auth: req.headers.authorization })
                        ...
                        if (req.method === 'GET') {
                            ...
                            return res.redirect(
                                `${req.protocol}://login.${host.substring(hostSubdomainEnd)}`,
                            )
                        } else {
        ...
        methods.forEach((method) => {
            ...
            if (secure) {
                this.app[method](endpoint, isAuthenticated, this.requestHandler(response))
            } else {
    ...
    run(
        ...
        if (this.isSecure()) {
			...
			serverPort = this.app.get('sslport')
            server = this.__ssl(serverOpts)
        } else {
			this.config.protocol = 'http'
			server = http.createServer(serverOpts, this.app)
		}
        ...
        const serverStartErrorHandler = (error, message) => {
        ...
        if (!!server) {
            server.listen(serverPort, started).on('error', serverStartErrorHandler)
        } else {
			this.log.error('app cannot start', {serverOpts})
		}
    }
