...
/// Utilities
const { getRootPath, log, logger, merge, getValuesFromObjectOrDefault, mkdirp, setInterval } = require('./util')(
    config.AppRoot,
)
...
class Sexpress {
    ...
    routeSecureSubdomainRequest(endpoint, response, method = 'get') {
        return this.routeSubdomainRequest(endpoint, response, method, true)
    }

    routeSubdomainRequest(endpoint, response, method = 'get', secure = false) {
        ...
        if (secure) {
            this.app[method](
                ...
                this.wrapRequestHandler(response),
            )
        } else {
