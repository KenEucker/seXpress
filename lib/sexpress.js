...
class Sexpress {
	...
	run(
		...
		if (!this.config.noSSL && !!this.config.ssl && !!this.config.ssl.passphrase) {
			const serverOpts = {
				cert: this.config.ssl.certificateFilename,
				key: this.config.ssl.certificateKeyFilename,
				passphrase: this.config.ssl.passphrase,
			}

			log.info('configuring SSL using certificate information', serverOpts)
			httpsServer = http.createServer(serverOpts, this.app)
		}
		...
		if (!!httpsServer) {
			httpsServer.listen(this.app.get("port"), () => {
				log(
					`App listening on: htts://localhost:${this.config.port}`
				)
			})
		}
	}
