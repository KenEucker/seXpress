...
class Sexpress {
    ...
    requestHandler(handler, restrictedSubdomains = [], skipLogging = false) {
        return (req, res, next) => {
            ...
            if (
				...
				console.log({no: this.config.headless, subdomain, yes: this.config.routing.indexControllerName})
                if (
    ...
    isValidSubdomain(subdomain, validSubdomains = [], ignoreSubdomains = []) {
        ...
        if (!isNotRestricted)
            console.log({ subdomain, validSubdomains, configuredSubdomains, subdomainIsConfiguredCoreModule })
