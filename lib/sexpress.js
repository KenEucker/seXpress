...
class Sexpress {
	...
	setConfiguration(config) {
		...
		if (fs.existsSync(this.config.contentFolder)) {
			...
			contentFiles.forEach((contentFile) => {
				...
				} else if (contentFileExtension === "pem") {

					this.config.ssl = this.config.ssl || {}

					if (contentFileName === "cert") {
						this.config.ssl.hasCertificate = true
						this.config.ssl.certificateFilename = contentFile
					} else if (contentFileName === "key") {
						this.config.ssl.hasCertificateKey = true
						this.config.ssl.certificateKeyFilename = contentFile
					}
				}
			})
		}
	...
	run(
		...
		if (!this.config.noSSL && !!this.config.ssl) {
			serverOpts.cert = this.config.ssl.certificateFilename
			serverOpts.key = this.config.ssl.certificateKeyFilename
			serverOpts.passphrase = this.config.ssl.passphrase

			log.info('configuring SSL using certificatte information', serverOpts)
		}
		const server = http.createServer(serverOpts, this.app)
