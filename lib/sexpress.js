...
class Sexpress {
    ...
    setConfiguration(config) {
		...
		const authorEmail = this.config.author ? this.config.author.split(' ')[0] : this.config.name.replace(' ', '.')
		this.config.author = {
			name: authorEmail,
			email: this.config.author ? util.extractEmails(this.config.author)[0] : `${authorEmail}@${this.config.host}`
		}
