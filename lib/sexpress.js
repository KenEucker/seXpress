...
class Sexpress {
        ...
        if (this.middlewares.redis && this.middlewares.redis.server) {
			...
			if (!callback) {
				...
				redisClient.mget(redisKeys, (e, keyVals) => {
					if (err) {
						this.log.error(`error getting redis keys values`, {error: e, keys, redisKeys})
						return
					}
					console.log({keyVals})
    ...
    async stop() {
		...
		if (this.config.preserve) {
			...
			const preservedConfigFilePath = join(this.config.folders.configFolder, `preserved.config.json`)

			if (typeof this.config.preserve === 'array') {
				/// TODO: save the config values that are present in the list
				this.config.preserve.forEach((opt) => {
					preservedConfig[opt] = this.config[opt]
				})
			} else {
				/// TODO: save all config values
				preservedConfig = this.config
			}
