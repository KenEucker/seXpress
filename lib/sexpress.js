...
class Sexpress {
    ...
    getCoreOpts(moduleName = '', overrides = {}, initial) {
        ...
        // console.log({defaults, fromConfig, overrides})
    ...
    renderTemplate(template, data, res, engine) {
        ...
        if (this.config.rendering.enabled && fs.existsSync(viewTemplate)) {
            this.log.debug('rendering template', { data, viewTemplate, locals: res.locals })
    ...
    __debug() {
		console.log({sslOpts: this.config.ssl.opts})
        const reloadServer = reload(this.app, { https: this.config.ssl.enabled ? this.config.ssl.opts : undefined })
