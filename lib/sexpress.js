...
class Sexpress {
	...
	crypto(secret = null) {
		...
		return {
			crypto: this.middlewares.encrypt.md5,
			decrypt: this.middlewares.encrypt.decrypt.bind(null, secret || this.config.secret),
			encrypt: this.middlewares.encrypt.encrypt.bind(null, secret || this.config.secret),
		}
	}
