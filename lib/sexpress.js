...
class Sexpress {
    ...
    _renderingModule(supportRendering = true) {
        ...
        if (this.config.supportRendering) {
            ...
            this.app.engine(`sexy`, this.sexyRenderingEngine.bind(this))

            /// Set view engine to our ejs and sass engine
            this.app.set('view engine', `sexy`)
    ...
    registerController(controller, name = '') {
        ...
        const viewEngine = !!controller.engine ? controller.engine : 'sexy'
    ...
    sexyRenderingEngine(templateFilePath, options, callback) {
        const self = this

		templateFilePath = templateFilePath.indexOf('.ejs') === -1 ? `${templateFilePath}.ejs` : templateFilePath 
        return ejs.renderFile(templateFilePath, options, (err, templateRendered) => {
            if (err) return callback(err)

            let rendered = templateRendered
            const templateSass = templateFilePath.replace('.ejs', '.scss')

            if (fs.existsSync(templateSass)) {
                const sassRender = sass.renderSync({ file: templateSass })
                const css = sassRender.css
                self.log.debug(
                    err
                        ? 'erorr rendering scss'
                        : 'scss rendered and attached to rendered template',
                    {
                        templateFilePath,
                        templateSass,
                        err,
