...
class Sexpress {
    ...
    setConfiguration(config) {
        ...
        Object.keys(this.config.subdomains).forEach((subdomain) => {
			...
			console.log({defaults: this.config.defaults, subdomainConfiguration, mergedSubdomainConfiguration})
            ...
            // this.authTokens[subdomain] = subdomainAuthConfiguration
    ...
    getLoginUrl(req, subdomain) {
		let protocol, host, port = this.config.port
    ...
    requestHandler(handler, subdomains, skipLogging = false) {
        ...
        const checkSubdomains = (subdomain) => {
            ...
            const ignoreSubdomains = ['api', 'login']
            const coreModules = Object.keys(this.core).filter(
                (m) => ignoreSubdomains.indexOf(m) === -1,
            )
            let reject = false
