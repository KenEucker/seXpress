        ...
        return Promise.all(this._initPromise).then(
            async function RunSexpress() {
                ...
                if (this.config.ssl.enabled) {
                    ...
                    if (httpsServer) {
                        httpsServer
                            .listen(this.config.ssl.port, () => {
                                this.log.status(
                                    `\n💃 *{${this.config.name}}* @ ${this.config.protocol}://${
                                        this.config.host
                                    }${
                                        this.config.ssl.port !== 80 || debuggingLocalhost
                                            ? `:${this.config.ssl.port}`
                                            : ''
                                    } 💃 \n\n`,
                                )
                            })
                    ...
                    if (this.config.port !== 80 || debuggingLocalhost) {
                        httpServer = null
                    }
                }
                        ...
                        .listen(this.config.port, () => {
                            this.log.status(
                                `\n💃 *{${this.config.name}}* @ ${this.config.protocol}://${
                                    ...
                                    this.config.port !== 80 || debuggingLocalhost
                                        ? `:${this.config.port}`
                                        : ''
                                } 💃 \n\n`,
                            )
