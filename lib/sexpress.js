...
class Sexpress {
    ...
    constructor(opts = {}) {
        ...
        config = this.util.merge(
            ...
            debug && fs.existsSync(debugFilename)
                ? this.util.merge(require(debugFilename), {
                      host: 'localhost',
                      port: 8080,
                      debug: true,
                  })
    ...
    getAvilableSSOProviders(subdomain) {
        ...
        Object.keys(this.config.authentication).forEach((authenticationName) => {
            const subdomainAuthOpts = this.authTokens[subdomain] || this.authTokens.default
            if (
                subdomainAuthOpts[authenticationName] &&
                subdomainAuthOpts[authenticationName].opts
            ) {
                if (subdomainAuthOpts[authenticationName].opts.clientID) {
        ...
        return Promise.all(this._initPromise).then(
            async function RunSexpress() {
                ...
                if (this.config.ssl.enabled) {
                    ...
                    if (!debuggingLocalhost && !httpsServer) {
                        httpServer = null
                    }
                }
                ...
                if (httpServer) {
                    ...
                    const httpServerPort = debuggingLocalhost ? 8080 : this.config.port
                    httpServer
