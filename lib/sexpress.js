...
class Sexpress {
    ...
    init() {
        ...
        this.initHost(this.config.host)
    ...
    initCookies(cookieOpts = {}) {
        ...
        cookieOpts = cookieSession(
            util.merge(
                {
                    name: this.config.appName,
                    key: this.config.appName,
                    keys: ['domain', 'maxAge'],
                    secret: this.config.security
    ...
    getLoginUrl(req, subdomain) {
        ...
        if (typeof req === 'string') {
            ...
            loginHost = req.hostname
            const hostSubdomainEnd = loginHost.indexOf('.') + 1
            loginHost = loginHost.substring(hostSubdomainEnd).replace('login.')
            protocol = req.protocol
        }

        return `${protocol}://login.${loginHost}${port !== 80 ? `:${port}` : ''}`
    }
