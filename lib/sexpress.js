...
class Sexpress {
	...
	run(
		...
		if (!this.config.noSSL && !!this.config.ssl && !!this.config.ssl.passphrase) {
			try {
				if (this.config.ssl.strategy === 'letsencrypt') {
					...
					certDirectoryFiles.forEach((domain) => {
						...
						const isDirectory = fs.lstatSync(path).isDirectory(domainPath)

						if (isDirectory) {
							certficates[domain] = {}

							certficates[domain].key = fs.readFileSync(path.join(domainPath, 'privkey.pem'))
							certficates[domain].cert = fs.readFileSync(path.join(domainPath, 'fullchain.pem'))
						}
					})
