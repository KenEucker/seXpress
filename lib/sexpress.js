...
class Sexpress {
    makeArt() {
        return new Promise((r) => {
			(() => {
				...
				const sexpressPackageJson = tryRequire.resolve('sexpress/package.json') ? require('sexpress/package.json') : {
					version: '0.0.0',
				}
				...
				console.log(
					asciiArtLogo({
						...
						.emptyLine()
						.right(sexpressPackageJson ? `seXpress version ${sexpressPackageJson.version}` : '')
						.render()
					)
    ...
    initMiddlewares(middlewares = {}) {
        ...
        middlewaresNames.forEach((middleware) => {
			...
			typeof this.config.middlewares[middleware] !== 'undefined' ? (typeof this.config.middlewares[middleware] === 'boolean'
                    ? { enabled: this.config.middlewares[middleware] } : this.config.middlewares[middleware])
                    : {}
	...
	getRedisKeys(keys = '*') {
		if (this.middlewares.redis && this.middlewares.redis.server) {
			return this.middlewares.redis.client().keys(keys)
		}
    ...
    isAuthenticated(req = {}) {
        ...
        if (!requestIsAuthenticated && req.passport) {
            ...
            console.log({
                ...
                isAuthenticatedMethoExists: req.isAuthenticated,
            })
            // console.trace()
	...
	async stop() {
		...
		if (this.middlewares.redis) {
			await this.middlewares.redis.close()
		}
