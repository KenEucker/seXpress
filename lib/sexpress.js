...
class Sexpress {
    ...
    isValidRequestOrigin(req, origin, subdomain) {
		...
		const checkAliasesAgainstOrigin = () => {
			let index = 0, valid = false
			const aliases = !this.config.subdomains[subdomain] ? [] : this.config.subdomains[subdomain].aliases || []
		...
		const originIsCorrectAliasedSubdomain = originIsCorrectSubdomain ? true : checkAliasesAgainstOrigin()
        const originIsValid = originIsCorrectSubdomain || originIsCorrectAliasedSubdomain || localhostPortIsTheSameForDebugging
