...
class Sexpress {
    ...
    requestHandler(handler, subdomains, skipLogging = false) {
        ...
        return (req, res, next) => {
            ...
            if (!skipLogging || this.config.debug) {
				...
				const reqIsOptionOrGet = ['options', 'get'].indexOf(req.method.toLocaleLowerCase()) !== -1
    ...
    run(
        started = () => {
            this.log.info(
                `\n 🤖 \x1b[42m\x1b[30m Ready \x1b[0m\x1b[0m {${this.config.appName}} is listening on: ${this.config.protocol}://${this.config.host}:${this.config.port}`,
            )
        },
    ) {
		this.log.status(`\n💃	running sexpress on port: ${this.config.port}\n`);
        ...
        const errorHandler = (error, message) => {
            message = `❗️  Encountered fatal error [${error.code}]${message ? ` - ${message}` : ''}: `
            switch (error.code) {
                case 'EADDRINUSE':
