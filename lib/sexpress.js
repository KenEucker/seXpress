...
const _getHostUri = (opts, host, protocol = '') => {
	const fullHost = host.indexOf(':') !== -1 ? host : `${host}${opts.port !== 80 ? `:${opts.port}` : ''}`
...
class Sexpress {
    ...
    _apiModule() {
        if (fs.existsSync(this.config.controllersFolder)) {
            ...
            this.route('/api/swagger.json', (subdomain, req, res, host) => {
				...
				if (subdomain !== this.config.indexControllerName) {
					servers.push({
						url: _getHostUri(this.config, host, req.protocol),
					})
				} else {
					Object.keys(this.config.subdomains).forEach((subdomain) => {
						// if (subdomain === this.config.indexControllerName) return

						servers.push({
							url: _getHostUri(this.config, `${subdomain}.${host}`, req.protocol),
						})
					})
				}

                const swaggerSpec = this.getSwaggerSpec(this.config, { servers })
    ...
    _authenticationModule() {
        ...
        const jwtOpts = {
            jwtFromRequest: this.config.jwtFromRequest || ExtractJwt.fromAuthHeaderAsBearerToken(),
            secretOrKey: this.config.secretOrKey || 'secret',
            issuer: this.config.issuer || this.config.host,
            audience: this.config.audience || this.config.host,
        }

        passport.use(
            new LocalStrategy(function (username, password, done) {
                console.log('yuoooooo', { username, password })
                if (username === 'test') return done(null, { name: test })
    ...
    getSwaggerSpec(opts, overrides) {
		...
		if (opts.apiSecurity) {
			...
			if (typeof opts.apiSecurity !== 'boolean') {
				Object.keys(securitySchemas).forEach(strategy => {
					if (opts.apiSecurity.indexOf(strategy) === -1) delete securitySchemas[strategy]
				})
				openApiDefinition.security = opts.apiSecurity.reduce((o, s) => { o[s] = [] ; return o }, {})
			} else if(opts.apiSecurityAll) {
				openApiDefinition.security = Object.keys(securitySchemas).reduce((o, s) => { o[s] = [] ; return o }, {})
			}

			openApiDefinition.components =  { securitySchemas }
		}
    ...
    initApiSecurity(strategies = [], jwtOpts = {}) {
		const allStrategies = [
			'basic',
			'bearer',
		]

		strategies = typeof strategies === 'boolean' ? allStrategies : strategies
		...
		strategies.forEach((strategy) => {
			switch(strategy) {
				...
				case 'bearer':
					jwtOpts.jwtFromRequest = jwtOpts.jwtFromRequest || ExtractJwt.fromAuthHeaderAsBearerToken()
    ...
    initPassport(opts = {}) {
		...
		this.initApiSecurity(this.config.apiSecurity)
    }
