...
class Sexpress {
            ...
            if (!this.config.middlewares) {
                ...
                middlewareFiles.forEach((middleware) => {
					...
					if (middlewareName !== 'index') {
						this.config.middlewares[middlewareName] = defaultEnabledMiddlewares.indexOf(middlewareName) !== -1
					}
                })
            }
    ...
    initMiddlewares(middlewares = {}) {
        ...
        middlewaresNames.forEach((middleware) => {
			...
			const middlewareConfiguredAsBoolean = typeof this.config.middlewares[middleware] === 'boolean'
            const middlewareOpts =
                typeof this.config.middlewares[middleware] !== 'undefined'
                    ? middlewareConfiguredAsBoolean ? { enabled: this.config.middlewares[middleware] }
            ...
            this.middlewares[middleware] = middlewareOpts.enabled ? new middlewareClass(middlewareOpts) : new middlewareNotSupported()
        })
    ...
    async stop() {
        ...
        Object.keys(this.middlewares).forEach(async (middleware) => {
            await (this.middlewares[middleware].close
                ? this.middlewares[middleware].close()
                : Promise.resolve())
        })
