...
class Sexpress {
    ...
    setConfiguration(config) {
        ...
        this.config.authentication =
            typeof this.config.authentication === 'object'
                ? this.config.authentication
                : { 
					enabled: typeof this.config.authentication === 'undefined' ? false : this.config.authentication
				}
    ...
    getCoreOpts(moduleName = '', overrides = {}, initial) {
		...
		const fromConfig = this.config[moduleName] || {}
		const moduleOpts = util.merge.all([initial, fromConfig, overrides])
