...
class Sexpress {
    ...
    init() {
        return util.promiseMe(async (done) => {
            ...
            this.log.debug(
                `\t\tI promise to ${this.config.run ? 'run' : 'wait'} all night.\n\t\t${
                    this.config.appPhrase
                }\n\n`,
            )
            this.log.status(`\nsexpress -> {${this.config.appName}} 💃\n`)
            ...
            this.log.debug(`sexpress application {${this.config.appName}} initialized`, this.config)
    ...
    setConfiguration(config) {
        ...
        this.config.folders.contentFolder = path.join(this.config.folders.publicFolder, 'content')
		this.config.folders.viewsFolder = path.join(this.config.folders.controllersFolder, 'views')
        ...
        if (fs.existsSync(this.config.folders.contentFolder)) {
            ...
            contentFiles.forEach((contentFile) => {
                ...
                if (contentFileExtension === '.html') {
                    const html = fs.readFileSync(path.join(this.config.folders.contentFolder, contentFile), 'utf8')
        ...
        return util.promiseMe(async (done) => {
            ...
            if (modules) {
                for (let i = 0; i < order.length; ++i) {
                    ...
                    const moduleInitPromise = modules[module].bind(this)()
    ...
    getCoreOpts(moduleName = '', overrides = {}, initial) {
        if (typeof initial === 'boolean') {
            ...
            initial = {
                enabled:
                    typeof this.config[moduleName] === 'boolean' ? this.config[moduleName] : true,
            }
        }

        const moduleOpts = util.merge3(initial, this.config[moduleName], overrides)
    ...
    run(
        started = () => {
            this.log.status(
                `\n{${this.config.appName}} @ ${this.config.protocol}://${this.config.host}:${this.config.port} 💃 \n\n`,
            )
        },
    ) {
