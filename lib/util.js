...
const getValuesFromObjectOrDefault = (names, input = {}, defaults = {}, final = {}, defaultsOverride = false) => {
	const out = {...input}
    names = names || Object.keys(defaults)

    names.forEach((name) => {
		if (defaultsOverride) {
			out[name] = typeof defaults[name] !== 'undefined'
			? defaults[name]
			: !!input[name]
			? input[name]
			: final[name]
			...
			out[name] = !!input[name]
				? input[name]
				: typeof defaults[name] !== 'undefined'
				? defaults[name]
				: final[name]
		}
    })
