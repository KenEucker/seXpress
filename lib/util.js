...
const injectIntoString = (haystack, needle, thread, marker = '%%MARKER%%', wrapper = s => s, before = true) => {
	...
	haystack =
		haystack.indexOf(marker) !== -1
			? haystack
			: `${marker}${haystack}`
	haystack = haystack.replace(
		marker,
		before
			? `${wrapper(thread)}${needle}`
			: `${needle}${wrapper(thread)}`,
	)
