...
const getSubdomainPrefix = (opts, req, returnAlias = false) => {
    const isLocalHost = opts.host === 'localhost'
    const subdomains = req.hostname.split('.')
    const indexSubdomain = subdomains[0] === opts.host ? opts.indexControllerName : subdomains[0]
	
	return returnAlias ? indexSubdomain : getSubdomainFromAlias(opts, indexSubdomain)
}
...
const getHostBaseUrl = (opts, host, protocol = '', subdomain, fullHost) => {
    let hostname = host
...
const getHostUri = (opts, host, protocol = '', append = '/api', fullHost) => {
    ...
    if (subdomain === 'api' || subdomain === opts.indexControllerName) {
        Object.keys(opts.subdomains).forEach((sub) => {
            ...
            if (sub === opts.indexControllerName) {
                sub = ''
                description = `Base ${opts.appName} application URI`
            } else if (ignoreSubdomains.indexOf(sub) !== -1) {
                return
            }

            servers.push({
                url: getHostUri(opts, `api.${host}/`, protocol, sub),
                description,
            })
        })
    } else if (subdomain !== opts.indexControllerName) {
        servers.push({
            url: getHostUri(opts, `api${host.replace(subdomain, '')}/`, protocol, subdomain),
            description: opts.subdomains[subdomain].description,
        })
    }

    /// And finally, add the index /api
    if (!opts.secureApiDocs) {
        servers.push({
            url: getHostUri(opts, host, protocol, '/api'),
            description: `Internal ${opts.appName} application URI`,
        })
    }
