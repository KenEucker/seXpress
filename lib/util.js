...
const getSubdomainPrefix = (opts, req, returnAlias = false) => {
    ...
    baseSubdomain =
        opts.host.indexOf(baseSubdomain) === -1
            ? baseSubdomain
            : opts.templating.indexControllerName
...
const getServers = (
    ...
    ignoreSubdomains.push(opts.templating.indexControllerName)
    const externalServerDescription = `External ${opts.appName} application URI`
    const internalServerDescription = `Internal ${opts.appName} application URI`

    if (subdomain === 'api' || subdomain == opts.templating.indexControllerName) {
        // if (subdomain !== 'api') {
        // 	servers.push({
        ...
        Object.keys(opts.subdomains).forEach((sub) => {
            ...
            if (sub === opts.templating.indexControllerName) {
                sub = ''
                apiHost = opts.host
